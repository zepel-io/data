...
function getRelativeSizeOf(base, component) {
  const { absoluteSize, gzipSize } = component;
  const { absoluteSize: totalSize, gzipSize: totalGzipSize } = base;

  return `${per(absoluteSize, totalSize)} / ${per(gzipSize, totalGzipSize)} (gzip)`;
}
